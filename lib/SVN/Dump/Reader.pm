...
sub read_record {
    ...
    $fh->eat_all_newlines_before_header();

    return if eof($fh); # no more records?
...
# Some svndumpfilter'd records have extraneous newlines...
sub eat_all_newlines_before_header {
    my $fh = shift;
    return if eof($fh);
    while (my $next_char = $fh->getc) {
        last if eof($fh);
        if ( $next_char ne "\n" ) {
